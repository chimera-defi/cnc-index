...
contract StratYieldRedirectAAVE is BaseStrategy {
    ...
    IAaveIncentivesControllerExtended public constant INCENTIVES_CONTROLLER =
        IAaveIncentivesControllerExtended(
    ...
    constructor(address _vault) public BaseStrategy(_vault) {
        ...
        ILendingPool lendingPool = ILendingPool(
            ADDRESS_PROVIDER.getLendingPool()
        );
    ...
    function setMinPrice(uint256 newMinWMATICToWantPrice) external onlyKeepers {
        require(
            newMinWMATICToWantPrice >= 0 && newMinWMATICToWantPrice <= MAX_BPS
        );
    ...
    function balanceOfRewards() public view returns (uint256) {
        ...
        uint256 totalRewards = INCENTIVES_CONTROLLER.getRewardsBalance(
            assets,
            address(this)
        );
        return totalRewards;
    ...
    function _claimRewardsAndGetMoreWant() internal {
        ...
        if (checkSlippageOnHarvest) {
            minWantOut = maticToWant(maticToSwap).mul(minWMATICToWantPrice).div(
                    MAX_BPS
                );
        }
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        if (WMATIC_TOKEN.balanceOf(address(this)) > 0) {
            WMATIC_TOKEN.safeTransfer(
                _newStrategy,
                WMATIC_TOKEN.balanceOf(address(this))
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 priceInMATIC = IPriceOracle(priceOracle).getAssetPrice(
            address(want)
        );
